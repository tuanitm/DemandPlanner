...
export default function BOMPage() {
  ...
  const columns: Column<BOMEntry>[] = [
    { key: 'raw_material_item_code', header: 'Component Code', width: '160px' },
    ...
    { key: 'rm_type', header: 'Type', width: '150px', render: (r) => {
      const rm = items.find(i => i.item_code === r.raw_material_item_code);
      return rm ? <span>{rm.item_type}</span> : <span style={{ color: 'var(--color-text-muted)' }}>—</span>;
    }},
          ...
          <div style={{ maxHeight: 320, overflowY: 'auto' }}>
            {filteredFG.length === 0 ? (
              ...
              <div key={item.item_code} onClick={() => handleSelectFG(item.item_code)}
                ...
                <span style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-text-muted)', flexShrink: 0 }}>{item.item_type}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            <>
              <div style={{ marginBottom: 'var(--space-4)', display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
                ...
                {(() => {
                  ...
                  return item ? <span style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-text-muted)' }}>{item.item_type}</span> : null;
                })()}
              </div>
