...
export default function ProductsPage() {
  ...
  const [iForm, setIForm] = useState({ item_group_code: '', item_code: '', item_partner_code: '', item_name: '', item_for_name: '', uom: 'PCS', item_type: 'Finished Goods', status: 'Active', lead_time_days: 14 });
  ...
  const handleSaveH = async () => {
    ...
    try {
      ...
      addToast('success', 'Product hierarchy created'); setShowHModal(false); fetchHierarchy();
  ...
  const handleSaveI = async () => {
    ...
    try {
      ...
      addToast('success', 'Item created'); setShowIModal(false); fetchItems();
    } catch (e) { addToast('error', 'Save failed', (e as Error).message); }
    finally { setSaving(false); }
  };
  ...
  const openCreateI = () => { setIForm({ item_group_code: '', item_code: '', item_partner_code: '', item_name: '', item_for_name: '', uom: 'PCS', item_type: 'Finished Goods', status: 'Active', lead_time_days: 14 }); setShowIModal(true); };
  ...
  return (
    <div className="animate-in">
      ...
      <Modal isOpen={showHModal} onClose={() => setShowHModal(false)} title="New Product Group" size="lg"
        footer={<><button className="btn btn-secondary" onClick={() => setShowHModal(false)}>Cancel</button><button className="btn btn-primary" onClick={handleSaveH} disabled={saving}>{saving && <span className="loading-spinner" />}Create</button></>}>
        ...
        <div className="form-row form-row-2">
          <div className="form-group"><label className="form-label">Group Code *</label><input className="form-input" placeholder="e.g. GRP-01" value={hForm.item_group_code} onChange={(e) => setHForm({ ...hForm, item_group_code: e.target.value })} /></div>
      ...
      {/* Create Item Modal */}
      <Modal isOpen={showIModal} onClose={() => setShowIModal(false)} title="New Item / SKU" size="lg"
        footer={<><button className="btn btn-secondary" onClick={() => setShowIModal(false)}>Cancel</button><button className="btn btn-primary" onClick={handleSaveI} disabled={saving}>{saving && <span className="loading-spinner" />}Create</button></>}>
        <div className="form-row form-row-2">
          ...
          <div className="form-group"><label className="form-label">Item Code *</label><input className="form-input" placeholder="e.g. SKU-001" value={iForm.item_code} onChange={(e) => setIForm({ ...iForm, item_code: e.target.value })} /></div>
        </div>
