...
import { Plus, Trash2, ShoppingCart, Factory } from 'lucide-react';
...
export default function OrdersPage() {
  ...
  const handleCreatePO = async () => {
    ...
    try {
      ...
      addToast('success', 'Purchase Order created'); setShowPoModal(false); fetchPO();
  ...
  const handleCreateMO = async () => {
    ...
    try {
      ...
      addToast('success', 'Production Order created'); setShowMoModal(false); fetchMO();
    } catch (e) { addToast('error', 'Failed to create', (e as Error).message); }
    finally { setSaving(false); }
  };
  ...
  const statusBadge = (s: string) => {
    const cls = s === 'Completed' ? 'badge-success' : s === 'In Progress' ? 'badge-warning' : s === 'Confirmed' ? 'badge-info' : 'badge-danger';
    ...
    <>
      ...
      {tab === 'po' && (<>
        <div style={{ display: 'flex', gap: 'var(--space-3)', justifyContent: 'flex-end', marginBottom: 'var(--space-4)' }}>
          ...
          <button className="btn btn-primary" onClick={() => setShowPoModal(true)}><Plus size={16} /> Add PO</button>
        </div>
      ...
      {tab === 'mo' && (<>
        <div style={{ display: 'flex', gap: 'var(--space-3)', justifyContent: 'flex-end', marginBottom: 'var(--space-4)' }}>
          ...
          <button className="btn btn-primary" onClick={() => setShowMoModal(true)}><Plus size={16} /> Add MO</button>
        </div>
      ...
      {/* PO Modal */}
      <Modal isOpen={showPoModal} onClose={() => setShowPoModal(false)} title="Create Purchase Order" size="md">
        ...
        <div className="modal-footer">
          ...
          <button className="btn btn-primary" onClick={handleCreatePO} disabled={saving || !poForm.po_number || !poForm.item_code}>
            {saving ? <><span className="loading-spinner" /> Saving...</> : 'Create PO'}
          </button>
        </div>
      </Modal>

      {/* MO Modal */}
      <Modal isOpen={showMoModal} onClose={() => setShowMoModal(false)} title="Create Production Order" size="md">
        <div className="form-row form-row-2">
        ...
        <div className="modal-footer">
          ...
          <button className="btn btn-primary" onClick={handleCreateMO} disabled={saving || !moForm.mo_number || !moForm.item_code}>
            {saving ? <><span className="loading-spinner" /> Saving...</> : 'Create MO'}
          </button>
        </div>
      </Modal>
